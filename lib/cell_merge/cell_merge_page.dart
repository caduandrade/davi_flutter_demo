...
class CellMergePage extends DemoFluPage {
  final ValueNotifier<CellCollisionBehavior> _cellCollisionBehavior =
      ValueNotifier(CellCollisionBehavior.overlap);
  final ValueNotifier<RowSpanOverflowBehavior> _rowSpanOverflowBehavior =
      ValueNotifier(RowSpanOverflowBehavior.cap);
  ...
  Widget _cellCollisionBehaviorChooser() {
    return Card(
        ...
        child: Column(
          children: [
            ListTile(title: Text("CellCollisionBehavior"), dense: true),
  ...
  Widget _cellCollisionBehaviorItem(
      CellCollisionBehavior item, String description) {
  ...
  Widget _rowSpanOverflowBehaviorChooser() {
    return Card(
        ...
        child: Column(
          children: [
            ListTile(title: Text("RowSpanOverflowBehavior"), dense: true),
            _rowSpanOverflowBehaviorItem(RowSpanOverflowBehavior.cap,
                'Adjusts the rowSpan to fit within the available rows, limiting the span.'),
            _rowSpanOverflowBehaviorItem(
                RowSpanOverflowBehavior.error,
                'Throws an error if the cell'
                's rowSpan exceeds the available number of rows.')
          ],
        ));
  }

  Widget _rowSpanOverflowBehaviorItem(
      RowSpanOverflowBehavior item, String description) {
  ...
  void buildSections(BuildContext context, PageSections sections) {
    Listenable listenable =
        Listenable.merge([_cellCollisionBehavior, _rowSpanOverflowBehavior]);
    ...
    sections.widget(
        (context) => Wrap(children: [
              IntrinsicWidth(child: _cellCollisionBehaviorChooser()),
              IntrinsicWidth(child: _rowSpanOverflowBehaviorChooser())
            ]),
        listenable: listenable);

    sections.widget(
        (context) => CellMergeOverflowExample(
            collisionBehavior: _cellCollisionBehavior.value,
            rowSpanOverflowBehavior: _rowSpanOverflowBehavior.value),
        title: 'Example with 2 data rows:',
        listenable: listenable)
